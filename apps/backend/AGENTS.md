...
- **Invite capacity and approval are server-resolved; volunteers cannot self-promote.** Only `JOINED` counts toward `filledCount` / seats. `joinRequiresApproval` on shift/event (default false) sends volunteer join/accept to `AWAITING_ADMIN_APPROVAL`. Waitlist promotion is FIFO by invite `createdAt` on that instance when a `JOINED` seat frees. Non-admin actors may request `JOINED` / `WAITLIST_JOINED` only from volunteer join-resolve sources (`ADMIN_INVITED` / `VOLUNTEER_REJECTED` / `VOLUNTEER_CANCELLED`) via `volunteerMayRequestInviteStatus` — never from `AWAITING_ADMIN_APPROVAL` or `WAITLIST_JOINED`. (invite-status-model / VOLI-1139)
