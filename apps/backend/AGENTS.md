...
- **Observability (Sentry)**: app code never imports `@sentry/nestjs` directly — inject `ObservabilityService` (`src/shared/observability/`) for `captureException` / `startSpan` / `setUser` / `withIsolationScope`. Application metrics go through the `metrics` facade from `@repo/observability` (backend-only; the frontend SDK has `enableMetrics: false`). `src/instrument.ts` is the only SDK init and must stay the first import in `main.ts` — it also wires `nodeProfilingIntegration()` (`SENTRY_PROFILES_SAMPLE_RATE`, relative to sampled transactions). Expected domain errors (`*GraphQLError` with codes in `EXPECTED_GRAPHQL_ERROR_CODES` from `@repo/observability`) are filtered out of Sentry by `SentryExceptionFilter`; `InternalServerGraphQLError` and unexpected errors are reported.
