...
- **Ending a membership hard-deletes org invites.** `leaveMembership` and `removeMembership` share one path: after deleting the membership row, hard-delete that user's `event_invites`, `shift_invites`, and `shift_instance_invites` for every unit in the same organization (all statuses). Other organizations and other users are untouched. This is complete removal, not `ADMIN_REJECTED`. Time entries, form submissions, and remaining memberships in other units of the same org are not part of this path yet. (VOLI-1109)
