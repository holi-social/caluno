...
- **Required forms are target-typed, not org-unit-specific.** `RequiredFormService` operates on a `RequiredFormTarget` (`targetType` / `targetId`). `ORGANIZATION_UNIT` is the only supported target today; shift/event targets are reserved for future expansion. Callers must not pass a raw `organizationUnitId` to required-form checks.
