...
- **Auth-only cross-org queries for personal data** are a sanctioned exception to multi-tenancy rules 2 & 3. Queries like `myTimeEntries` are `@Session`-only (no `@Permissions`, no org context), self-scoped to `session.user.id`, and return the user's OWN data across orgs. Permitted only where `userId` comes exclusively from the session — never a GraphQL arg (IDOR prevention). (my-time)
- **`TimeEntry.shiftInstance` field resolver is org-scoped** — it throws without an `organizationUnitId` in context. Org-agnostic read paths must use a dedicated flat read model (`MyTimeEntry`, mapped from preloaded relations), not reuse `TimeEntry`. (my-time)
