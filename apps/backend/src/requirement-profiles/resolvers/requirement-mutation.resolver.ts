...
export class RequirementMutationResolver {
  ...
  async createRequirement(
    ...
    const item = await this.requirementService.create(input);
  ...
  async updateRequirement(
    ...
    const item = await this.requirementService.update(id, input);
    return this.requirementMapper.toModelOrThrow(item);
  }

  @Permissions(PERMISSIONS.REQUIREMENT_DELETE)
  @Mutation(() => Requirement)
  async deleteRequirement(@Args('id') id: string): Promise<Requirement> {
    const item = await this.requirementService.delete(id);
    return this.requirementMapper.toModelOrThrow(item);
  }
}
