...
UPDATE "event_invites" SET "status" = 'INVITED' WHERE "status" = 'PENDING';--> statement-breakpoint
DROP TYPE "event_invite_status";--> statement-breakpoint
CREATE TYPE "event_invite_status" AS ENUM('INVITED', 'ACCEPTED', 'REJECTED', 'CANCELLED', 'SELF_JOINED');--> statement-breakpoint
...
UPDATE "shift_instance_invites" SET "status" = 'INVITED' WHERE "status" = 'PENDING';--> statement-breakpoint
UPDATE "shift_invites" SET "status" = 'INVITED' WHERE "status" = 'PENDING';--> statement-breakpoint
DROP TYPE "shift_invite_status";--> statement-breakpoint
CREATE TYPE "shift_invite_status" AS ENUM('INVITED', 'ACCEPTED', 'REJECTED', 'CANCELLED', 'SELF_JOINED');--> statement-breakpoint
