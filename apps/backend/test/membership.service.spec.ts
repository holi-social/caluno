...
describe('MembershipService', () => {
  ...
  describe('getInvitableMemberships', () => {
    it('includes a member whose membership is on an ancestor unit', async () => {
      const rootMember = await createUser(db);
      const { organization, type } = await createOrganizationWithType(
        db,
        `Ancestor Org ${crypto.randomUUID()}`,
      );
      ...
      await addMembership(db, rootMember.id, root.id);

      const memberships = await service.getInvitableMemberships(child.id);

      expect(memberships.map((m) => m.userId)).toContain(rootMember.id);
    });

    it('excludes a member of an unrelated sibling unit', async () => {
      const siblingMember = await createUser(db);
      const { organization, type } = await createOrganizationWithType(
        db,
        `Sibling Org ${crypto.randomUUID()}`,
      );
      const root = await createUnit(db, {
        organizationId: organization.id,
        typeId: type.id,
      ...
      await addMembership(db, siblingMember.id, sibling.id);

      const memberships = await service.getInvitableMemberships(child.id);

      expect(memberships.map((m) => m.userId)).not.toContain(siblingMember.id);
    });

    it('does not duplicate a user with memberships on multiple ancestor levels', async () => {
      const user = await createUser(db);
      const { organization, type } = await createOrganizationWithType(
        db,
        `Dedup Org ${crypto.randomUUID()}`,
      );
      const root = await createUnit(db, {
        organizationId: organization.id,
        typeId: type.id,
      ...
      await addMembership(db, user.id, child.id);

      const memberships = await service.getInvitableMemberships(child.id);

      expect(memberships.filter((m) => m.userId === user.id).length).toBe(1);
    });
  });
